...
The project requires persistent storage for the domain entities described
below. The storage technology, persistence library, schema definitions, and
migration mechanism are implementation choices.

**Topic**

- A topic requires a unique title of 64 characters or fewer.
- A topic has an optional short description of 255 characters or fewer.

**Article**

- An article requires a title of 255 characters or fewer.
- An article requires content of at least 255 characters.
- An article has exactly one author.
- An article is associated with one or more topics.
- `created_at` is a date and time recorded when the article is created and
  never changes afterwards.
- `updated_at` is a date and time refreshed whenever the article is modified.
- An article becomes publicly available immediately after creation. A draft or
  publication workflow is outside the core project scope.

**Comment**

- A comment belongs to exactly one article.
- A comment has exactly one author.
- A comment requires message text.
- `created_at` is a date and time recorded when the comment is created.

**Topic preference**

A user's relationship with a preferred topic is represented by a
`UserTopicPreference` association.

- A `(user, topic)` pair must be unique.
- The existence of the association means the topic is preferred by the user.
- `notifications_enabled` records whether the user is subscribed to
  notifications for that preferred topic.
- `notifications_enabled` defaults to `false`.
- Unsubscribing sets `notifications_enabled` to `false` but keeps the
  preference.
- Removing a topic from the preferred list removes the association and its
  notification state.
- A topic must be preferred before notifications can be enabled for it.

The notification delivery mechanism is an implementation concern; this
association only defines the persistent preference and subscription state.

**Relationships**

- `Article` and `Topic` have a many-to-many relationship. Every article has at
  least one topic; a topic may be associated with zero or more articles.
- `User` and `Article` have a one-to-many relationship. Every article has one
  author; a user may author zero or more articles.
- `Article` and `Comment` have a one-to-many relationship. Every comment
  belongs to one article; an article may have zero or more comments.
- `User` and `Comment` have a one-to-many relationship. Every comment has one
  author; a user may author zero or more comments.
- `User` and `Topic` are connected through `UserTopicPreference`.

**Default article ordering**

Unless a feature explicitly defines another ordering, article collections must
be ordered by `created_at` in descending order. If two articles have the same
timestamp, implementations must apply a stable deterministic tie-breaker.
...
erDiagram
    ...
    topic }|--o{ article: categorizes
    article ||--o{ comment: contains
    user ||--o{ article: writes
    user ||--o{ comment: writes
    user ||--o{ user_topic_preference: has
    topic ||--o{ user_topic_preference: selected_in
...
>   topics. `created_at` descending remains the primary ordering key. For
>   articles with the same creation timestamp, articles matching more preferred
>   topics come first. The final result must remain deterministic.
...
Add a stable human-readable slug to each article.

- A slug is required and unique when this challenge is implemented.
- The slug is generated when an article is created and does not change when
  the article title is edited later.
- The base format is the slugified article title followed by the creation date
  in `YYYY-MM-DD` form. For example, "Sample article" created on 2023-03-24
  produces `sample-article-2023-03-24`.
- If the base slug already exists, append the first available numeric suffix,
  starting with `-2`, then `-3`, and so on.
- Existing articles must receive slugs according to the same rules when the
  feature is introduced.
- Article detail URLs should use the slug as their article identifier.
